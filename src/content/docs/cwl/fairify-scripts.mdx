...
The goal is to be able to share the data analysis with as much context and functionality as possible. This should enable yourself or other researchers to **replicate, reproduce or reuse** it in other contexts or on other data of the same type.
...
2. **Separate data analysis from the data**
    - Avoid hard-coding paths to inputs or outputs such as data, parameters or dependencies
    - The smaller and more modular the process, the easier it will be to reuse it or combine it in multi-step pipelines

3. Collect **information about  dependencies**, such as external packages and tools
    - Record the script language itself
    - Record the packages, libraries, tools
    - Record their versions
    - Record available resources
      - Official citation
      - URLs to tool or developer website, docs, GitHub / GitLab
      - Unique tool IDs from registries such ([bio.tools](https://bio.tools/)) or RRIDs ([SciCrunch](https://scicrunch.org/resources/about/resource), identifiers.org)  

4. Un-couple processes from environment
    - The result of a script should not depend on where it is run.
    - To achieve this, provide or reuse existing containers that include as much of the above information as required to make the use independent 
