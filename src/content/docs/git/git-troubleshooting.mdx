...
### Download a single (LFS) file of an ARC via GitLab API using a token

- `<project_id>`: numerical project ID (check three-dots menu of ARC in top-right corner)
- `<your_token>`: a project access token with at least `Reporter` role and `read_repository` scope (Settings -> Access toke)
- `<url_encoded_path>`: full path to file, URL-encoded (replace `/` with `%2F`, e.g. `path/to/data.raw` to `path%2Fto%2Fdata.raw`)
- `<branch>`: the branch from which to download (usually `main`)
- `<fileName>`: output name of the file (`data.raw`)

```bash
curl --header "PRIVATE-TOKEN: <your_token>" \
     --location \
     "https://git.nfdi4plants.org/api/v4/projects/<project_id>/repository/files/<url_encoded_path>/raw?ref=<branch>&lfs=true" \
     -o <fileName>
```

:::note
The `&lfs=true` at the end can be omitted for non-LFS files.
:::
