...
The commit messages in the DataHUB should be the same as in the local ARC. 
...
<Steps>
   1. Open ARC in command window or terminal, e.g. via ARCitect --> Tools --> Command Window
   2. Run `git status`
</Steps>

Check [this section](#git-status-1) for details.
