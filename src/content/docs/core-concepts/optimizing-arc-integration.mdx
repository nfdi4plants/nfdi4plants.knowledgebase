...
10. ### Create and share reusable computational workflows
    ...
    - So, for code-based data analysis in your lab, you and your team members will already appreciate that common directory structure plus version-control via [Git](/nfdi4plants.knowledgebase/fundamentals/version-control-git/).
...
A typical scenario could look like this: You create your ARC on you personal computer, add structure with studies and assays as you see fit and upload the ARC to the DataHUB to have a save copy and be able to share it with collaboration partners. You acquire some large dataset, which you do not want to store or analyze on your computer. Instead you keep this on a larger workstation in the lab or a server or high performance computing (HPC) cluster, where you would also run data analysis workflows. With the DataHUB as the meeting point, you can keep your ARC in sync across these locations. Simply download the ARC from the DataHUB to your server, add the dataset from there and synchronize it back to the DataHUB. Consider your ARC stored in the DataHUB as the "ground truth" – even if you delete the ARCs on your local computer or the server, it will still be available in the DataHUB including the change history from those source locations.

:::note

Many labs collaborate on local or cloud storage servers: multiple team members edit the same project folder, 

Especially in the lab context, it is recommended to collaborate via the DataHUB logic. This 
