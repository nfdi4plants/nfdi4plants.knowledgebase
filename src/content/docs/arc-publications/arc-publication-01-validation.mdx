...
- **(2)**: The _`invenio` badge_ is used to open a request for ARC publication via [ARChigator](/nfdi4plants.knowledgebase/datahub/arc-publications/datahub-arc-publications-archigator).
    - If an ARC fails the `invenio` CQC it is not eligible for publication. Clicking the badge will show the failed tests and their details, which can be used to fix the issues in the ARC metadata.
    - If an ARC passes the `invenio` CQC, it is eligible for publication. Clicking the badge will open the ARChigator interface to submit a request for publication (RFP) of the ARC.
