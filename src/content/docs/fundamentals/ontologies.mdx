...
{/* TODO: replace OWL examples with code blocks? */}
...
![img1](@images/fundamentals/plant-ontology-image.png)
...
Ontologies can be used for many different purposes. As such, there are also multiple languages which an ontology can be written in, as well as many formats. Some of the most relevant for our context include the Web Ontology Language (OWL) and Open Biomedical Ontologies (OBO) format. OBO was mainly used by the biomedical community (see section OBO Foundry) before the rise of OWL and today is considered a dialect of OWL.
...
Since the OBO format does not cover all aspects that can be included in OWL formatted ontologies, conversion between the formats can be problematic. Generally, it is possible to convert OBO files into OWL files using conversion tools such as the [OWL API](https://owlcs.github.io/owlapi/), since OBO can be interpreted as a reduced OWL form. However, when trying to convert OWL files to OBO files, there can be significant information loss when concepts foreign to OBO (such as DatatypeProperties mentioned above) are used.


## OBO Foundry
The Open Biological and Biomedical Ontologies (OBO) Foundry is a team of scientists interested in the development, collection and maintenance of ontologies for the biological and biomedical domain of life sciences. 
...
3. Each term should have a unique identifier by using a unique prefix (such as GO) followed by a numerical ID, thereby resulting in a unique OBO Foundry URL.
...
12. The naming of elements such as classes, subclasses etc. should follow naming conventions.
13. Major changes to an ontology SHOULD be announced before the release.
16. The ontology needs to be maintained.
19. The definition of a term MUST always mean the same. Minor changes are acceptable, as long as they don't change the meaning of the definition.
