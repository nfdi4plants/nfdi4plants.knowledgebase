...
<Card icon="pen" title="ARCitect">
    ...
    2. At the bottom of the right panel, click on the <btn>+</btn> right next to the `Metadata` sheet to add one sheet for each lab process.
...
<Card icon="pen" title="ARCitect">
    ...
    2. Import the assay files from the demo data into the two assays in your ARC:
        demo data -------> | ARC
        --- | ---
    ...
    2. In the bottom-right corner click <btn> < </btn> to open a panel on the right.
    3. On top of the panel, navigate to the templates tab.
    4. In the "Add template(s) from file" section, select ARCtrl and click <btn>Upload protocol</btn>.
    5. Navigate to and select the file `AssayTemplate_Proteomics_MS.json` and click <btn>open</btn>.
