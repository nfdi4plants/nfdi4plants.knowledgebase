...
To add [building blocks](/nfdi4plants.knowledgebase/swate/building-blocks) to your actual template, you need to switch from the "Metadata" sheet to the "New Table" sheet on the bottom left. Then you click "Add Building Block" (<Icons.BuildingBlock />) in the <a href={SwateOverview.src} target="_blank">Sidebar</a> or <a href={SwateOverview.src} target="_blank">Widgets</a>. You can add building blocks as simple terms or units at your selected column in the table by clicking the "Add Column" button.

</TabItem>
<TabItem label="Excel">

To add [building blocks](/nfdi4plants.knowledgebase/swate/building-blocks) to your actual template, you you click "Add Building Block" (<Icons.BuildingBlock />) in the <a href={SwateOverview.src} target="_blank">Sidebar</a>.
You can add building blocks as simple terms or units at your selected column in the table by clicking the "Add Column" button.
...
Save your template by clicking on the disc symbol in the top right corner. This will download an xlsx-file. Please adjust the name of the file to correspond to the name of the template, but use underscores instead of spaces and add the version number (e.g. DNA_extraction_v1.1.7.xlsx).
The file should be saved in the folder corresponding to your organization within your local clone of the Swate template registry repository (swate-template-registry > templates > community > "your organization"). Here, please create a subfolder named after your template and save the template in that folder. If you are updating an existing template, simply add the new version of the file to the existing folder. The old file can remain in the same folder.
