---
title: In-Memory Editing of Online ARC
...
This guide will show case how to

1. Retrieve and parse the ARC RO Crate from PLANTdataHUB
1. Update the Metadata of the the ARC ISA Entities to information from external sources (e.g. BrAPI Server)
1. Update Material Processing complying to MIAPPE 
1. Add Observation data in a MIAPPE compliant way
1. Update the ARC in the PLANTdataHUB with local changes
...
for (const brapiStudy of brapiStudies) {
  ...
  */
  ...
  try {
    ...
    // Creates the table with its columns
    ...
    // Fill the table with rows following the previously defined table columns
    for (let ou of observationUnits) {
...
We have now sucessfully updated our ARC. The last step is to export it to the PLANTdataHUB. We will be using the Gitlab REST API for this. To write the ARC, we will make use of Contracts.
ARCtrl allows to create Write Contracts for the ISA files using the `GetWriteContracts()` function. We will use those contracts and the `Xlsx` library to create virtual files that are not stored on the local filesystem to add actions to the Gitlab Commit payload by base64 encoding the Investigation, Studies and Assays. Afterwards we also add make sure that the Datamaps and Datafiles are also included as base64 encoded strings in the commit actions.
Finally, the commit is send to the PLANTdataHUB.
...
// Previously created datafiles array with Contract-like
// structure (see section Create Result Data File with Datamap).
