...
This guide demonstrates how to create individual datamaps for each Assay using CSV tables located within the `datasetFolder`.

## Use-case

Researchers often need to store specific metadata—such as the measurement units for a particular analyte (e.g., chlorophyll a)—directly within a datamap. 
This workflow facilitates the automated generation of these datamaps for selected Assays by extracting the relevant measurement parameters from the CSV files in an Assay's dataset.
