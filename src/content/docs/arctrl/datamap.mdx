...
The Datamap files used to annotate data files are modelled in ARCtrl as `Datamap` objects.
...
The `Datamap` itself is a table with fixed columns. Each row in the Datamap represents and annotates a Data Fragment of a data file and is modelles as an `DataContext` object. The `Datamap` therefore consists of a collection of `DataContext` objects.
...
2. Then we set the `DataMap` of the first assay to our `datamap`.
...
arc.ISA.Value.GetAssayAt(0).DataMap <- Some datamap
...
arc.ISA.GetAssayAt(0).DataMap = datamap
