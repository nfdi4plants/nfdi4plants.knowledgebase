...
<Steps>
1. In the `Tokens tab`, select your eLabFTW Instance or add Instance [1]
...
<Steps>
1. In the `Home tab`, select [1] your eLabFTW Experiment/Resource or enter the corresponding ID [2]. Use commas to separate multiple IDs.  
    ![](@images/elab2arc/elab2ARC-home-page.png)
5. Click **Submission** [3] to fetch entries. 
3. **Check status** [4] provides detailed information about the submission. 
4. **Preview** [5] shows more information about the eLabFTW entry. Here you can also select linked Resources/Experiments for conversion. 
...
<Steps>
1. In the `ARCs tab`, select the ARC into which the elabFTW entry should be converted. 
2. You can use an existing ARC by clicking the **Convert to assay** button [1] or create a new ARC with the **Create a new ARC** button [2]. 
4. Use **Start Conversion** button [3] to start the elab2ARC conversion. 
