...
1. Navigate to the ARC in the DataHUB
1. Create a new milestone to bundle the issues (left sidebar → Plan → Milestones)
    ...
    **Title**
        ...
        :::note
        If the title is not `ARC publication revision` the import of the here provided issue list will fail, since all issues are assigned to this milestone.
        :::
...
1. Import issues 
    ...
    - left sidebar → Plan → Issues
    - three-dots menu in the top-right corner → Import CSV
    - Select the `review-arc-issues.csv` file and import it
    - Wait a short moment. You will receive a notification once the import is finished. The issues should now be visible in the issue list and assigned to the milestone "ARC publication revision".

</Steps>

:::tip
In case there's an issue with the import or you want to bulk close the imported issues, you can use the `bulk edit` feature in the top-right of the issue menu. This allows you to select multiple issues and perform actions on them. For instance, to close them: Select the issues, change the status to "Close", select "Update selected".
:::
